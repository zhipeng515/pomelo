...
var Component = function(app, opts) {
  ...
  this.blacklist = [];
  this.oncelist = {};
  this.onceExpiry = opts.onceExpiry || 60  * 1000;
  this.msgExpiry = opts.msgExpiry || 30 * 1000;
...
var verifyMessage = function(self, session, msg) {
  ...
  var pubKey = self.getPubKey(session.id);
  if (!pubKey) {
    logger.error('could not get public key, session id is %s', session.id);
    return false;
  }
  ...
  if (!pubKey.verifyString(message, sig)) {
    logger.error('rsa verify failed [%s, %s, %s]', self.app.serverId, message, sig);
    return false;
  }

  var once = msg.body.__once__;
  if (!once) {
    logger.error('receive sign data from client has no once param [%s]', self.app.serverId);
    return false;
  }
  var onceTime = self.oncelist[once];
  if (!onceTime || onceTime + self.onceExpiry < Date.now()) {
    self.oncelist[once] = Date.now();
  } else {
    logger.error('reply attack [%s, %d, %d]', self.app.serverId, once, onceTime);
    return false;
  }

  var expiry = msg.body.__expiry__;
  if (!expiry) {
    logger.error('receive sign data from client has no expiry param [%s]', self.app.serverId);
    return false;
  }

  if (expiry + self.msgExpiry < Date.now()) {
    logger.error('message has expired [%s, %d]', self.app.serverId, expiry);
    return false;
  }

  delete msg.body.__once__;
  delete msg.body.__expiry__;
