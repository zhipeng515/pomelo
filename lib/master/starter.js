 ...
 * @return {Void}
 */
starter.runImage = function(env, server, cb) {
    var dockerCmd = util.format('%s run -d --name %s --net=%s', Constants.COMMAND.DOCKER, server.id, process.env.NET_MODE || 'host');
    var cmdKey = '';
    if (server.port !== undefined) {
        dockerCmd += util.format(' -p %s:%s', server.port, server.port);
    }
    if (server.clientPort !== undefined) {
        dockerCmd += util.format(' -p %s:%s', server.clientPort, server.clientPort);
    }
    dockerCmd += ' -e NODE_ENV=' + env;
    dockerCmd += ' ' + process.env.IMAGE_NAME;
    var args = '';
    for(cmdKey in server) {
        if(cmdKey === Constants.RESERVED.CPU) {
            cpus[server.id] = server[cmdKey];
        }
        args += util.format('%s=%s ', cmdKey, server[cmdKey]);
    }
    dockerCmd += util.format(' node app %s', args);
    starter.dockerrun(dockerCmd, server.host, cb, null, cb);
}
...
starter.run = function(app, server, cb) {
    ...
    if (process.env.IMAGE_NAME !== undefined) {
        ...
        cmd = util.format('%s inspect -f \'{{.State.Running}}\' %s', Constants.COMMAND.DOCKER, server.id);
        starter.dockerrun(cmd, server.host,
            // error callback
            function(errmsg) {
            },
            // success callback
            function(state) {
                state = state.replace(/[\r\n]/g, '');
                if (state == 'false') {
                    cmd = util.format('%s rm %s', Constants.COMMAND.DOCKER, server.id);
                    starter.dockerrun(cmd, server.host, null, null, function(code) {
                        if(code == null) {
                            starter.runImage(env, server, cb);
                        } else {
                            logger.error('docker rm failed ' + code);
                        }
                    });
                } else if (state == 'true') {
                    logger.error(cmd + ' ' + state);
                } else {
                    logger.info(cmd + ' ' + state);
                    starter.runImage(env, server, cb);
                }
            },
            // exit callback
...
starter.kill = function(pids, servers) {
    ...
    for(var i = 0; i < servers.length; i++) {
        ...
        // if (process.env.IMAGE_NAME !== undefined) {
        //     cmd = util.format('%s rm -f %s', Constants.COMMAND.DOCKER, server.id);
        //     console.log("start.kill.......%s", cmd);
        //     starter.dockerrun(cmd, server.host, cb);
        // } else
        if(utils.isLocal(server.host)) {
            var options = [];
...
/**
 * Use ssh to run docker command.
 *
 * @param {String} cmd command that would be executed in the remote server
 * @param {String} host remote server host
 * @param {Function} error callback function
 * @param {Function} success callback function
 * @param {Function} exit callback function
 *
 */
starter.dockerrun = function(cmd, host, error, success, exit) {
    var sshCmd = 'ssh -o StrictHostKeyChecking=no -p ' + process.env.SSH_PORT + ' ' + process.env.SSH_USER + '@' + host + ' ' + cmd;
    logger.info('Executing ' + cmd + ' on ' + host + ':' + process.env.SSH_PORT);
    execProcess(sshCmd, host, [], error, success, exit);
    return;
};
...
var spawnProcess = function(command, host, options, cb) {
    ...
    if(env === Constants.RESERVED.ENV_DEV) {
        ...
        var prefix = (command === Constants.COMMAND.SSH || command === Constants.COMMAND.SSHPASS) ? '[' + host + '] ' : '';
...
/**
 * Fork child process to exec command.
 *
 * @param {String} command
 * @param {Object} options
 * @param {Callback} callback
 *
 */
var execProcess = function(command, host, options, error, success, exit) {
    var child = null;
    options = options.slice(0);
