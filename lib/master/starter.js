...
starter.run = function(app, server, cb) {
  ...
  var cmd, key;
  if (server[Constants.RESERVED.CONTAINER] !== undefined) {
    ...
    cmd += ' ' + server[Constants.RESERVED.CONTAINER]
    delete server[Constants.RESERVED.CONTAINER];
    var args = util.format('env=%s', env);
    for(key in server) {
...
starter.dockerrun = function(cmd, host, cb) {
    var sshpass = Constants.COMMAND.SSHPASS + ' -p ' + '\'' + process.env.SSH_PASSWORD + '\'' + ' ssh -o StrictHostKeyChecking=no ' + process.env.SSH_USER + '@' + host + ' ' + cmd;
    logger.info('Executing ' + cmd + ' on ' + host + ':22');
