...
starter.run = function(app, server, cb) {
  ...
  if (process.env.IMAGE_NAME !== undefined) {
    cmd = util.format('%s run -d --name', Constants.COMMAND.DOCKER, server.id);
    if (server.port !== undefined) {
      cmd += util.format(' -p %s:%s', server.port, server.port);
    }
    if (server.clientPort !== undefined) {
      cmd += util.format(' -p %s:%s', server.clientPort, server.clientPort);
    }
    cmd += ' -e NODE_ENV=' + env;
    cmd += ' ' + process.env.IMAGE_NAME;
    var args = '';
    for(key in server) {
      if(key === Constants.RESERVED.CPU) {
          cpus[server.id] = server[key];
      }
      args += util.format('%s=%s ', key, server[key]);
    }
    cmd += util.format(' node app %s', args);
 ...
 *
 */
starter.dockerrun = function(cmd, host, cb) {
    var sshpass = Constants.COMMAND.SSHPASS + ' -p ' + '\'' + process.env.SSH_PASSWORD + '\'' + ' ssh -o StrictHostKeyChecking=no -p ' + process.env.SSH_PORT + ' ' + process.env.SSH_USER + '@' + host + ' ' + cmd;
    logger.info('Executing ' + cmd + ' on ' + host + ':' + process.env.SSH_PORT);
    execProcess(sshpass, host, [], cb);
    return;
};
 ...
 *
 */
var execProcess = function(command, host, options, cb) {
    var child = null;
    options = options.slice(0);

    if(env === Constants.RESERVED.ENV_DEV || process.env.IMAGE_NAME !== undefined) {
